...
///         {DeployPausable} recipe so a facet test can prove `whenNotPaused`/`whenPaused` gate a real action
///         through the REAL diamond dispatch — never shipped, never part of a `run()` deploy.
