...
Install Foundry **v1.8.1** (forge, cast, anvil), Git, Bash, Python 3, and jq. CI uses that Foundry
release. The project currently compiles with Solidity 0.8.36; use the same compiler for snapshots.
...
checks compatibility within each declared namespace across upgrades.
...
inheritance mega-facet past the deployment size limit. Preserve old fields and append only where the
storage Action proves compatibility. Regenerate/review the snapshot locally, compare to a trusted
release commit, then propose the cut through Governor. If a cut runs a new initializer, use a strictly
...
For reproducible docs and the reusable guard, see the quickstart and the Action README in
`.github/actions/storage-layout/README.md`.
