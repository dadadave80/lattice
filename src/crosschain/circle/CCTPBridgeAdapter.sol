...
contract CCTPBridgeAdapter is ICCTPBridgeAdapter {
    ...
    /// @inheritdoc ICCTPBridgeAdapter
    function depositForBurnWithHook(uint256 amount, bytes calldata recipient, bytes calldata hookData)
        external
        virtual
    {
        CCTPBridgeAdapterLib.depositForBurnWithHook(amount, recipient, hookData);
    }
    ...
    /// @inheritdoc ICCTPBridgeAdapter
    function relayMessageWithHook(bytes calldata message, bytes calldata attestation) external virtual {
        CCTPBridgeAdapterLib.relayMessageWithHook(message, attestation);
    }
    ...
    /// @inheritdoc ICCTPBridgeAdapter
    function hookExecutor() external view virtual returns (address) {
        return CCTPBridgeAdapterLib.hookExecutor();
    }
    ...
    ///      `relayMessageWithHook(bytes,bytes)` 0x96692d34
    ///      `tokenMessenger()` 0x46117830
    ///      `usdc()` 0x3e413bee
    function exportSelectors() external pure virtual returns (bytes memory selectors) {
        selectors =
            hex"4736cc953d8f1160143854033f1f7a9f1a7a98e2f7d38221ef21b76fb9e064027b04c181a3e6f53529351b4596692d34461178303e413bee";
    }
}
