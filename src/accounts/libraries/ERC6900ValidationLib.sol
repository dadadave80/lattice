...
import {ERC6900ExecutorLib, ValidationCheckType} from "@lattice/accounts/libraries/ERC6900ExecutorLib.sol";
import {
...
import {SparseCalldataSegmentLib} from "@lattice/accounts/libraries/SparseCalldataSegmentLib.sol";
...
library ERC6900ValidationLib {
    ...
    {
        if (msg.sender != ERC4337ValidationLib.entryPoint()) revert IValidation6900.NotFromEntryPoint(msg.sender);
    ...
    function _validateUserOp(PackedUserOperation calldata userOp, bytes32 userOpHash) private returns (uint256) {
        if (userOp.callData.length < 4) revert IExecutor6900.UnrecognizedFunction(_selectorOf(userOp.callData));
        ...
        if (v.executionHooks.length() > 0) revert IValidation6900.RequireUserOperationContext();
    ...
    function _doUserOpValidation(
        ...
        for (uint256 i; i < n; ++i) {
            ...
            if (uint160(res) > 1) revert IValidation6900.UnexpectedAggregator(module, entityId, address(uint160(res)));
    ...
    function _execUserOpValidation(
        ...
        if (!v.validationFlags.isUserOpValidation()) {
            revert IValidation6900.UserOpValidationInvalid(module, entityId);
        }
