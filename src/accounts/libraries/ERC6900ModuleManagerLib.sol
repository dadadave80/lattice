...
library ERC6900ModuleManagerLib {
    ...
    {
        _authorizeConfig();
        if (module == address(0)) revert IModuleManager6900.NullModule();
        ...
        for (uint256 i; i < n; ++i) {
            ...
            if (e.module != address(0)) revert IModuleManager6900.ExecutionFunctionAlreadySet(selector);
            ...
            if (DiamondLib.selectorToFacet(DiamondLib.diamondStorage(), selector) != address(0)) {
                revert IModuleManager6900.ExecutionFunctionShadowsFacet(selector);
            }
            e.module = module;
        ...
        for (uint256 i; i < h; ++i) {
            ...
            if (!$._executions[mh.executionSelector].executionHooks.add(_hookKey(hookConfig))) {
                revert IModuleManager6900.ExecutionHookAlreadySet(hookConfig);
            }
        }
    ...
    function installValidation(
        ...
        for (uint256 i; i < hookCount; ++i) {
            ...
            if (hookConfig.isValidationHook()) {
                ...
                if (v.validationHooks.length > MAX_VALIDATION_ASSOC_HOOKS) {
                    revert IModuleManager6900.PreValidationHookLimitExceeded();
                }
        ...
        for (uint256 i; i < selCount; ++i) {
            if (!v.selectors.add(selectors[i])) {
                revert IModuleManager6900.ValidationAlreadySet(selectors[i], validationFunction);
            }
        }
    ...
    function uninstallValidation(
        ...
        if (hookDataCount > 0) {
            ...
            if (hookDataCount != valLen + execLen) revert IModuleManager6900.ArrayLengthMismatch();
            uint256 idx;
    ...
    function _addInterface(ERC6900ModuleManagerStorage storage $, bytes4 interfaceId) private {
        if (
            ...
            revert IModuleManager6900.InvalidInterfaceId(interfaceId);
        }
    ...
    function _onInstall(address module, bytes calldata data) private {
        ...
        try IERC6900Module(module).onInstall(data) {}
            ...
            revert IModuleManager6900.ModuleInstallCallbackFailed(module, reason);
        }
    }
    ...
    function _authorizeConfig() private view {
        if (msg.sender != address(this) && !AccessControlLib.hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            revert IModuleManager6900.UnauthorizedModuleConfig(msg.sender);
        }
    }
}
