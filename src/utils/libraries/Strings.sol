...
///         Upstream is MIT. Only the address→hex helpers are re-declared (the full Strings lib pulls in
///         Math/SignedMath/Bytes). Vendored subset — do not add an openzeppelin-contracts dependency.
/// @notice Address-to-string helpers, incl. EIP-55 checksummed hex (needed by gateway adapters that
///         address remote contracts by string, e.g. Axelar).
library Strings {
