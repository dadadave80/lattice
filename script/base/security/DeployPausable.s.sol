...
contract DeployPausable is BaseDeploy {
    ...
    /// @return cuts The facet cuts (ERC165 + AccessControl + Pausable).
    /// @return init The {PausableInit} initializer address.
    ...
    function buildCuts(address admin) public returns (FacetCut[] memory cuts, address init, bytes memory initCalldata) {
        ...
        cuts[2] = _cut(address(new Pausable()), "Pausable");
        init = address(new PausableInit());
        initCalldata = abi.encodeCall(PausableInit.init, (admin));
    }
