...
contract DeployDIAAdapter is BaseDeploy {
    ...
    /// @return cuts The facet cuts (ERC165 + AccessControl + DIAAdapter).
    /// @return init The {DIAAdapterInit} initializer address.
    ...
    function buildCuts(address admin) public returns (FacetCut[] memory cuts, address init, bytes memory initCalldata) {
        ...
        cuts[2] = _cut(address(new DIAAdapter()), "DIAAdapter");
        init = address(new DIAAdapterInit());
        initCalldata = abi.encodeCall(DIAAdapterInit.init, (admin));
    }
